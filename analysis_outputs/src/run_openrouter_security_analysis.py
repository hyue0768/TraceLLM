...
def normalize_event_name(name: str) -> str:
	"""Normalize event name for robust filename matching (case-insensitive, collapse spaces/punct)."""
	s = name.strip().lower()
	s = re.sub(r"[\s\-_]+", " ", s)
	s = re.sub(r"[^a-z0-9()\. ]+", "", s)
	s = s.strip()
...
def find_event_file(base_dir: Path, event_name: str, prefer_exts: Tuple[str, ...]) -> Optional[Path]:
	"""Find a file in base_dir whose basename (without extension) matches event_name.
	Priority:
	1) Exact normalized basename match
	2) Case-insensitive basename match
	3) Startswith match
	Only returns files with extension in prefer_exts if provided.
	"""
	...
	# Case-insensitive exact base match
...
def load_transaction_trace(trace_json_path: Path, max_chars: Optional[int] = None) -> Tuple[str, Optional[str]]:
	...
	if max_chars is not None and len(text) > max_chars:
		...
		text_display = text
	# Try to parse attacker address for {target_contract}
...
def main() -> None:
	...
	parser.add_argument("--event", required=True, help="事件名（与报告文件名一致，如 'Zunami Protocol'）")
	parser.add_argument("--trace-json", required=True, help="交易追踪 JSON 文件路径（attacker_trace_cli.py 输出）")
	parser.add_argument("--contracts-dir", default="/home/os/yuehuang/Contract_analysis_llm/contracts", help="contracts 报告目录（.md/.txt）")
	parser.add_argument("--mythril-dir", default="/home/os/zhuoer/mythril", help="mythril 报告目录（.md/.txt）")
	parser.add_argument("--api-base", default="https://openrouter.ai/api/v1", help="OpenRouter API Base URL")
	parser.add_argument("--model", default="google/gemini-2.0-flash-001", help="模型名")
	parser.add_argument("--api-key", default=os.getenv("OPENROUTER_API_KEY", ""), help="OpenRouter API Key（或用环境变量 OPENROUTER_API_KEY）")
	parser.add_argument("--referer", default=os.getenv("OPENROUTER_REFERER", ""), help="可选：HTTP-Referer 头")
	parser.add_argument("--title", default=os.getenv("OPENROUTER_TITLE", ""), help="可选：X-Title 头")
	parser.add_argument("--temperature", type=float, default=0.3, help="温度系数")
	parser.add_argument("--max-tokens", type=int, default=None, help="可选：最大输出 tokens")
	parser.add_argument("--max-chars-report", type=int, default=None, help="可选：对每个报告截断的最大字符数")
	parser.add_argument("--max-chars-trace", type=int, default=None, help="可选：对 trace JSON 截断的最大字符数")
	parser.add_argument("--output", default="", help="输出 Markdown 文件路径；不填则写到 ./analysis_outputs/<event>.md")

	args = parser.parse_args()

	if not args.api_key:
		raise SystemExit("必须提供 API Key（--api-key 或环境变量 OPENROUTER_API_KEY）")
